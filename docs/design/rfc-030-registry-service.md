...
- **Not a package search/discovery ranking algorithm** — the web UI's search is real and required (npm/crates.io parity), but *how* results are ranked (downloads, recency, relevance) is implementation detail this RFC does not prescribe.
...
- GET /packages/{name} — list all published versions of a package (for cohdl add/cohdl update to discover the latest exact version, and for the web UI's package page).
...
- Search/browse endpoints backing the web UI are real and required but their exact shape is implementation detail, not specified here (per Non-goals).
...
High. An AI author writing [dependencies] learns exactly one small, closed rule beyond RFC-029's existing exact-version grammar: a package name is either bare (official), @brand/name (manufacturer), or @contrib/name (community) — three shapes, each immediately legible, no memorized exception list. cohdl login/publish/add/remove/install/update are six small, single-purpose commands, each named for exactly one npm/cargo-equivalent action a model (or human) already has strong priors for.
...
- `cohdl check --json`/`cohdl build --json` gain no new fields from this RFC — registry interaction is a pre-compilation, CLI-level concern (like RFC-029's manifest/lock verification stage), not a compiler diagnostic.

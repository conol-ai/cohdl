...
- `polarity_marker cathode_pin N shape SHAPE` — `SHAPE` closed to `{band, arrow}`. `N` must be an already-declared `pad` number (the part's cathode/negative terminal), and the footprint must contain at least two distinct electrical pad numbers so the marker has another terminal to orient toward. Repeated physical placements of one number remain one terminal. `band` expands to a `line` primitive (a short, wide stroke) drawn immediately adjacent to pad `N`, perpendicular to the line between the two terminal pads — the conventional diode cathode-band silkscreen mark. `arrow` expands to a `polygon` primitive (a small filled triangle) pointing away from pad `N` toward the anode — the conventional "current flow" diode arrow convention.
...
2. **Marker-target existence and polarity orientation** — `pin_1_marker`/`polarity_marker`'s referenced pad number `N` must exist on the same `footprint`, and a polarity marker requires at least two distinct electrical pad numbers. Both are local, single-declaration lookups, not emergent whole-design or connectivity-graph facts. This is exactly the kind of check DR-006's classification logic assigns to the type system, not residual DRC — nothing here requires knowing anything beyond this one `footprint` declaration's own already-declared content.
...
- Marker-target pad existence (`pin_1_marker near pad N` / `polarity_marker cathode_pin N`) and the polarity marker's minimum of two distinct electrical numbers are checked immediately against the same `footprint`'s own already-declared pads — a single-declaration, non-emergent lookup, checked with the rest of the footprint-body validation.

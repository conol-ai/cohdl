...
outline and would read as 49 components locked at the board centre). Staging
positions are exact over the femto integers and byte-stable. With no outline
there is nothing to stage against, so `Component/Location` keeps the `(0,0)`
placeholder. Either way this is NOT a real placement — it is the input a

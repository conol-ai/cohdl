...
fn check_with_std(board_src: &str) -> (cohdl::pipeline::Checked, String) {
    ...
    let std_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("std/0.1.0/src");
...
fn example_build_matches_committed_golden_output() {
    ...
    for name in ["rpi-pico2", "openmicro"] {
        ...
        let proj = cohdl::project::load_project(&dir, Some(&root.join("std/0.1.0"))).unwrap();

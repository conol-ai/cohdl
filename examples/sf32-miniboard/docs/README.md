...
`mechanical/sf32-miniboard-outline.dxf` is generated by
`tools/make_board_outline_dxf.py` as a metric AutoCAD R2000 document. Its
60 x 40mm profile has 3mm corner radii and one closed `LWPOLYLINE` on
`Edge.Cuts`; the four M2 NPTH features are placed components rather than
additional outline loops. Regenerate it from the repository root with:

```sh
python3 tools/make_board_outline_dxf.py \
  examples/sf32-miniboard/mechanical/sf32-miniboard-outline.dxf \
  --width 60 --height 40 --radius 3
```

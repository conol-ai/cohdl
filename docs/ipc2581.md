...
CoHDL owns no board outline or stackup today — and footprint geometry
only where RFC-018 pad placements have been authored — so the document is
**logical-complete, physical-minimal**, and says so in three
...
SURFACE/THRU from the pad's plating); a footprint still at the RFC-017
placeholder stage (empty body) keeps the zero-size-polygon idiom.

**Exactly which authored pad geometry the IPC `Pin` subset OMITS** (review
R5-8), so "real footprint geometry" is not read as complete: the pad's
`layer` (top/bottom/through — the B1 `Pin` element carries only a
plating-derived SURFACE/THRU `type`, not the copper layer), the pad's
`drill` diameter (through-hole holes are not projected), and the
footprint's `silkscreen_ref`. The `.kicad_mod` projection carries all of
these; the IPC document does not. Projecting them faithfully needs the
schema's `LandPattern`/`PadStackDef`/layer-pad/hole structures — a
larger format-mapping effort tracked as future work, not a claim that the
source data is complete here. The `logical-complete,physical-minimal`
marker governs; a consumer must not treat the `Pin` subset as a full
padstack.

...
| E810 | malformed `mount_hole` (RFC-022) — a footprint's mechanical locating hole is ill-formed: a duplicate `mount_hole` number within one footprint, a non-`Length` (or out-of-range) offset/diameter, a non-positive diameter, or a `PLATING` value outside the closed set `{non_plated, plated}`. `mount_hole` numbers are their own namespace, never checked against `pad` numbers or the bound device's pins. |

...
# Status: v2 ground-up language redesign — Layer-1 foundation complete (2026-07-13)

The founding decisions below are locked and unchanged. Notes 2 through 10 were reset and rewritten from scratch on 2026-07-13 per GC-003: the language's conceptual model was redesigned around a new central thesis, strictness buys expressiveness — push correctness into the type system wherever a mistake is structural (Rust-inspired, not Rust-copied), and reserve DRC only for genuinely emergent/numeric checks. All seven P0 RFCs (units, pins, traits, DRC narrowing, designators, nested fns, generics) are now Accepted — the Layer-1 type-system foundation is formally specified end to end in note 10. The previous (v1) implementation at conol-ai/cohdl is being discarded/rewritten to match this design. See DR-005/DR-006 for the redesign's rationale. Note 9 (MVP Definition) has been reactivated now that the foundation is real — see its current content for the v0.1 scope line and demo scenario.

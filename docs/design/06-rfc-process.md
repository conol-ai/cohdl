...
Milestone (2026-07-19, second time this day): Tony directed a new RFC grounded directly in a real, already-built example — examples/openmicro/src/main.cohdl (a 13-key macropad) — which showed 42 near-identical inst lines (13 switches, 13 diodes, 29 addressable RGB LEDs, 4 mounting holes) and multiple uniform-fan-out net-member patterns (keyboard-matrix ROW/COL nets, one with a real stride: d1, d5, d9, d13). RFC-024 (Instance arrays and range references) is Accepted: inst NAME[START..=END]: Device declares a contiguous family of real, individually-addressable instances in one statement; NAME[RANGE | RANGE step STRIDE | i1, i2, ...].PIN inside a net's member list expands to an ordinary flat pin-reference list. Both mechanisms are defined purely as expansion sugar — the expanded form is checked identically to what an author would have hand-written, with zero new semantic checking beyond structural array-collision and out-of-range-index diagnostics. Explicitly scoped narrow: does not touch fn/place/decouple, and explicitly does not solve OpenMicro's own daisy-chain LED-chain wiring or arithmetic-derived per-instance place coordinates — both real, disclosed, deferred future work, not silently implied solved. Recorded as DR-030 (see note 7). Backlog is now RFC-001 through RFC-024, all Accepted.

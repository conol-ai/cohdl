...
- `FunctionMode/@comment` — `logical-complete,placement-staged,unrouted —
  layout not yet performed` (with `mode="USERDEF"`, deliberately not claiming
  a full assembly/fabrication data set);
...
`Pin` element per pad placement (`Location` + `RectCenter`/`Circle`/`Oval`,
`type` SURFACE/THRU and `mountType` `SURFACE_MOUNT_PAD`/`THROUGH_HOLE_PIN`).
...
and a `LayerFeature` per copper layer whose `Pad`s place each padstack at its
absolute board position (component transform + rotated pad offset), tied to
the component pin (`PinRef`) and net (`Set/@net`). This is what makes the
copper, holes, and ratsnest visible/routable in a consumer (Quilter) — the
finding in `.co/invalid-ipc2581.xml`. (The earlier "moot for an all-SMD board"
note was stale: rpi-pico2's USB + castellations are through-hole with real
0.44/1.0/1.35 mm drills.) Still not projected into the IPC document: the
footprint's `silkscreen_ref` (present in the `.kicad_mod`).

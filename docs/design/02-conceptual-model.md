...
| **Module** | A namespace grouping devices/parts/fns; controls visibility (`pub`). | **Formalized 2026-07-14 (RFC-016)** — a package's module tree mirrors its file tree under `src/`, rooted at `cohdl.toml`'s `[package] name`; no separate `mod` declaration. `pub` is now actually enforced, but only across package boundaries — intra-package visibility stays fully open, unchanged from the single-flat-scope MVP behavior. This was a real, load-bearing gap until RFC-016: CoHDL had exactly one flat global name bucket since the ground-up redesign began. |
...
| **Library** | A distributable Package (see Module above) carrying reusable `.cohdl` source, plus optional reference documents and footprints. | **New concept, 2026-07-14 (RFC-017, revised same day)** — no new mechanism beyond Module/Package; a Library is exactly a Package, distinguished only by intent (published for reuse). Reference documents attach via `#[doc("path")]` (zero compiler impact, same discipline as `#[intent(...)]`); footprints attach via a `footprint` symbol reference (not a path string), resolved through Module's own machinery. Manufacturer "best-practice" skills are explicitly deferred to a future concept, not yet designed. |
| **Footprint** | A named, resolvable declaration (`footprint`, peer of Device/Trait/Fn/Part) composed of `Pad` references placed at offsets, plus courtyard/silkscreen geometry. | **New concept, 2026-07-14 (RFC-017 → RFC-018, same day)** — RFC-017 first made footprint a resolvable symbol with an unspecified body; RFC-018 gave it real content, adopting Cadence Allegro's proven pad/footprint split (per Tony's direct design direction). The keyword itself never changed — `footprint` keeps its name from RFC-017, simply gaining real content for the first time (same role: what `part.footprint:` points to). |
| **Pad** | A reusable geometric primitive (`pad`) — shape, size, layer, plating — referenced (never inlined) by one or more Footprints. | **New concept, 2026-07-14 (RFC-018)** — the actual conceptual move Cadence's design demonstrates: a pad is defined once and shared across every footprint that needs it, so a correction to one pad's geometry propagates everywhere it's referenced, instead of drifting across copy-pasted numbers. Deliberately minimal scope (no per-layer-independent padstacks, no vias/thermal reliefs) — that richness has no board-outline/stackup context to mean anything against yet (RFC-015's still-open gap). (Same-day naming note: the first draft used the invented name `copad`; Tony corrected this to plain `pad`.) |
...
- Overlapping names. Still guarded — Trait / Device / Part / Instance / Net / Spec / Rule / Module / Fn / Design / Designator / Library / Footprint / Pad remains the canonical vocabulary (Library and Footprint added 2026-07-14 via RFC-017; Pad added 2026-07-14 via RFC-018; all other names unchanged, redesigned guarantees).

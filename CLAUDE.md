...
- `cargo run -- update <dir> [--dep NAME]` — re-resolve pins, rewrite `cohdl.lock` (RFC-029; also migrates a manifest with no `[dependencies]`)
...
- `std/` — the std package (RFC-029): an ordinary package, `cohdl.toml` (declares the version — the sole authority) + `src/`; only what the demo boards need
